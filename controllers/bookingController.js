...
exports.getCheckoutSession = catchAsync(async (req, res, next) => {
  ...
  const session = await stripe.checkout.sessions.create({
    ...
    // Always give in single line for safer side this will goto the Home Page View Router
    // success_url: `${req.protocol}://${req.get('host')}/?tour=${
    //   req.params.tourId
    // }&user=${req.user.id}&price=${tour.price}`,
    success_url: `${req.protocol}://${req.get('host')}/mybooking/?alert=Booking`,
    ...
    line_items: [
      {
        ...
        ], // images we need to metion server url.
